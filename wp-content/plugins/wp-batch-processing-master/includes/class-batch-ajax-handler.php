	...
	public function process_next_item() {
		...
		if ( $is_finished ) {
			...
			if ( is_wp_error( $response ) ) {
				$error_message = apply_filters( 'dg_batch_item_error_message', 'Error processing item with id ' . $next_item->id . ': ' . $response->get_error_message(), $next_item, $batch_id );
				wp_send_json_error( array(
				...
				$success_message = apply_filters( 'dg_batch_item_success_message', 'Processed item with id ' . $next_item->id, $next_item, $batch_id );
				wp_send_json_success( array(
