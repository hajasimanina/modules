(function($){
	...
	$(document).on('itemprocessed', function(e, response){
		...
		if ( $('.batch-process-current-item table#result') .length > 0 ){
            $('.batch-process-current-item table#result tbody').append(response.data.message);
		} else{
            $('.batch-process-current-item').append(response.data.message);
		}
