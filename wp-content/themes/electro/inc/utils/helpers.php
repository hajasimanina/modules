...
function require_once_files_in( $path , $recursive = false ) {
	if ( is_dir( $path ) ) {
		if ( $recursive ) {
			$nodes = glob( $path . '/*' );
			foreach ( $nodes as $node ) {
				if ( is_file( $node ) ) {
					...
					require_once_files_in( $node , $recursive );
				}
			}
		} else {
			$nodes = glob( $path . '/*.php' );
 ...
 * @return stdClass
 */
function electro_image_info( $attachment = null , $size = IMAGE_SIZE_PRODUCT ) {
	$element = new stdClass();
	if ( !empty( $attachment ) ) {
		list( $element->src ) = wp_get_attachment_image_src( $attachment['ID'] , $size );
		$element->srcset = wp_get_attachment_image_srcset( $attachment['ID'] , $size );
		$element->title   = $attachment['title'];
		$element->alt     = ( !empty( $attachment['alt'] ) ) ? $attachment['alt'] : $attachment['title'];
	} else {
		$element->src     = get_template_directory_uri() . '/assets/images/default.jpg';
		$element->src_set = get_template_directory_uri() . '/assets/images/default.jpg';
