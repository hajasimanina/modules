...
//enlever le p auto et br auto
//add_filter('tiny_mce_before_init','electro_tiny_mce_before_init',10,2);
function electro_tiny_mce_before_init($mceInit, $editor_id){
  ...
  //css rte BO
  $mceInit['content_css'] = get_template_directory_uri() . "/assets/css/rteBO.php" ;
...
add_filter( 'mce_buttons_2', 'electro_mce_buttons_2' );
function electro_mce_buttons_2( $buttons ) {
...
//acf wysiwyg
add_filter( 'acf/fields/wysiwyg/toolbars' , 'electro_acf_wysiwyg_toolbars'  );
function electro_acf_wysiwyg_toolbars( $toolbars )
{
...
add_action( 'wp_enqueue_scripts', 'electro_default_scripts' );
function electro_default_scripts(){
  //remplacer jquery par une version recente
  if(!is_admin()){
    wp_deregister_script('jquery');
    wp_register_script('jquery', get_template_directory_uri() .'/assets/js/jquery.min.js', false, '1.9.0',false);
    wp_enqueue_script('jquery');
  }

 /*wp_enqueue_script('jquery-lazyload', get_template_directory_uri() .'/assets/js/library/jquery.lazyload.min.js',
		array('jquery'), '1.8.0' , true);*/
}

//add_filter( 'the_content','electro_default_the_content',9);
/*function electro_default_the_content($html){
...
function wp_do_lazyload_wpimage($matches){
  $lazyimage = get_template_directory_uri() .'/assets/images/trans.gif';
